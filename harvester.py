                    ...
                    for key in credkey:
                        if key in env.name.lower():
                            if env.value_from is None:
                                ...
                                reported_issues.append({'name': env.name, 'value': env.value, 'container': container, 'pod': pod, 'namespace': pod_map[pod]['namespace'], 'key': key, 'source': 'podEnv'})
                            else:
                                if env.value_from.config_map_key_ref is None:
                                    ...
                                    reported_issues.append({'name': env.value_from.config_map_key_ref.key, 'value': tmp_configmap.data[env.value_from.config_map_key_ref.key], 'container': container, 'pod': pod, 'namespace': pod_map[pod]['namespace'], 'key': key, 'source': 'configmapEnv'})
                    ...
                    for key in credkey:
                        for tmp_env in tmp_mainifest['ContainerConfig']['Env']:
                            if key in tmp_env.split('=')[0].lower():
                                reported_issues.append({'name': tmp_env.split('=')[0], 'value': tmp_env.split('=')[1], 'container': container, 'pod': pod, 'namespace': pod_map[pod]['namespace'], 'key': key, 'source': 'manifestEnv'})
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--outfile', help='The file to write results (needs to be writable for current user)', required=False, default='harvester.json')
